const puppeteer = require(`puppeteer`),
      devices = require(`puppeteer/DeviceDescriptors`),
      fs = require(`fs`), 
      path = require(`path`);

async function extractBookList(extractData){
    //console.log(`INFO:Browser init...`);

    // 启动puppeteer
    const browser = await puppeteer.launch();
    
    // 创建新的标签页
    const page = await browser.newPage();

    // 创建UA仿真环境
    page.setUserAgent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4182.0 Safari/537.36");

    // 创建模拟的移动浏览器仿真环境
    await page.emulate(devices[`iPhone X`]);

    // 创建cookie的仿真环境
    await page.setCookie({
        name: '__utmv',
        value: '30149280.26378',
        domain: '.douban.com',
        path: '/',
        expire: '2024-10-24T03:34:54.000Z'
    }, {
        name: '__utmt',
        value: '1',
        domain: '.douban.com',
        path: '/',
        expire: '2022-10-25T03:44:54.000Z'
    }, {
        name: '__utmb',
        value: '30149280.2.10.1666668894',
        domain: '.douban.com',
        path: '/',
        expire: '2022-10-25T04:04:54.000Z'
    }, {
        name: '__utmz',
        value: '30149280.1666270868.2.2.utmcsr=book.douban.com|utmccn=(referral)|utmcmd=referral|utmcct=/',
        domain: '.douban.com',
        path: '/',
        expire: '2023-04-25T15:34:54.000Z'
    }, {
        name: '__utma',
        value: '30149280.1435678354.1666265164.1666312497.1666668894.4',
        domain: '.douban.com',
        path: '/',
        expire: '2024-10-24T03:34:54.000Z'
    }, {
        name: 'push_noty_num',
        value: '0',
        domain: '.douban.com',
        path: '/',
        expire: '2022-11-24T03:34:54.000Z'
    }, {
        name: '_pk_ref.100001.2939',
        value: '%5B%22%22%2C%22%22%2C1666668894%2C%22https%3A%2F%2Fbook.douban.com%2F%22%5D',
        domain: 'search.douban.com',
        path: '/',
        expire: '2023-04-25T15:34:54.000Z'
    }, {
        name: '_pk_ses.100001.2939',
        value: '*',
        domain: 'search.douban.com',
        path: '/',
        expire: '2022-10-25T04:04:54.000Z'
    }, {
        name: '_pk_id.100001.2939',
        value: '035f21ddd9acb30b.1666265164.4.1666668894.1666314518.',
        domain: 'search.douban.com',
        path: '/',
        expire: '2024-10-24T03:34:54.000Z'
    }, {
        name: 'ck',
        value: 'iyS3',
        domain: '.douban.com',
        path: '/'
    }, {
        name: '__utmc',
        value: '30149280',
        domain: '.douban.com',
        path: '/'
    }, {
        name: 'dbcl2',
        value: '"263786629:mXxTw0oBoO4"',
        domain: '.douban.com',
        path: '/',
        expire: '2022-11-19T11:29:57.058Z'
    }, {
        name: 'push_doumail_num',
        value: '0',
        domain: '.douban.com',
        path: '/',
        expire: '2022-11-24T03:34:54.000Z'
    }, {
        name: 'bid',
        value: 'kSu1TaBLA7g',
        domain: '.douban.com',
        path: '/',
        expire: '2023-10-20T11:26:03.443Z'
    });

    console.log(`DEBUG:`, extractData.url)

    // 访问需要访问的url
    await page.goto(extractData.url);

    // 考虑有些页面是通过js进行渲染，故延时几秒再获取dom内容
    if(extractData.jsRender){
        await page.waitFor(1000);
    }

    //console.log(`INFO:Browser process data...`);


    // 当前页面查询到的所有匹配记录的列表
    let list = [];

    // 设定列表的过滤规则
    const books = await page.$$(`.sc-bxivhb`);

    // 遍历列表对象，寻找每个对象内部的可用信息，如标题，地址，书籍的原始数据等
    for(let i = 0;i < books.length;i++){
        const title  = await books[i].$eval('.title-text', (element) => element.innerText);
        const href  = await books[i].$eval('.title-text', (element) => element.href);
        const meta  = await books[i].$eval('.meta', (element) => element.innerText);
        let pl  =  -1;
        let rating_nums = -1;

        try{
            // 因为不存在此节点语句就会报错，故捕获异常并保持字段为初值，即-1
            pl = await books[i].$eval('.pl', (element) => element.innerText);
            rating_nums  = await books[i].$eval('.rating_nums', (element) => element.innerText);
        }catch(e){ }

        // 将摘取的可用信息作为新的数据对象传入列表
        list.push({
            title: title,
            href: href,
            pl: pl,
            meta: meta,
            rating_nums: rating_nums
        });
    }

    // 实际返回的结果，一个唯一值
    let result;
    let highScore = 0;

    // 遍历列表中所有符合查询条件的记录，分数最高，且名称匹配记录
    for(let j = 0; j < list.length;j++){
        let t = list[j].title;
        let s = parseFloat(list[j].rating_nums);

        console.log(`DEBUG:`, j, extractData.originTitle, t, s)
        
        // 按照标题符合就算符合来进行过滤
        if((t == extractData.originTitle || t.indexOf(extractData.originTitle) >= 0)
        && s >= highScore) {
            highScore = s;
            result = list[j];
            console.log(`DEBUG:HIGH SCORE`, j, extractData.originTitle, t, s)
        }
    }



/*
    books.forEach(function(book){
        console.log('####', book)
    });
*/


    //console.log(`INFO:Browser process finish.`);

    // 关闭浏览器
    await browser.close();

    //console.log(`INFO:Browser closed.`);

    // 返回唯一值
    return result;
}



const REPEATED_BOOK_LIST = [
`C++ Templates（第2版 英文版）`,
`最後的問題 : 西方短篇科幻小說精選`,
`中华人民共和国国民经济和社会发展第十三个五年规划纲要`,
`DK艺术博物馆：世界名作全景导读`,
`第三方JavaScript编程`,
`结构化计算机组成`,
`Microsoft.NET框架程序设计`,
`Java安全编码标准`,
`唐山 : 一个世纪的故事`,
`数据结构与算法分析 : Java语言描述（英文版·第3版）`,
`数字逻辑应用与设计`,
`HR劳动争议经典管理案例`,
`复杂SOC设计`,
`计算机体系结构 : 计算机体系结构嵌入式方法`,
`山中小记：圆霖长老随缘开示`,
`做个不骂孩子的妈妈 : 亲子语言沟通对错280句`,
`程序设计方法（第2版）`,
`网络体系结构及设计`,
`性能之巅（第2版）：系统、企业与云可观测性`,
`Web 2.0界面设计模式`,
`怎样给孩子讲故事`,
`老舍作品选读`,
`UNIX系统编程: 通信、并发与线程`,
`多处理器编程的艺术（原书第2版）`,
`好奇宝贝科学思维系列绘本：这是谁的牙齿`,
`计算复杂性理论基础`,
`明天要遠足`,
`行为自动机研究 : 选择性综合神经模拟`,
`我们属龙`,
`Java Collections`,
`思维规律的研究`,
`黑客大曝光 : Web应用安全机密与解决方案（第2版）`,
`学会跟孩子说话`,
`明日边缘`,
`第一性－女人的天賦正在改變世界`,
`Linux服务器架设、性能调优、集群管理教程 : 实训与项目案例`,
`不对称陷阱 : 當別人的風險變成你的風險，如何解決隱藏在生活中的不對等困境`,
`认识大脑开发大脑 : 认识大脑开发大脑`,
`名画之谜 : 51幅世界杰作的秘密`,
`Java I/O (O'Reilly Java)`,
`強勢領導的迷思 : 從林肯到歐巴馬，我們到底想要哪一種政治領袖？`,
`Web Hacking : Attacks and Defense`,
`运动改变大脑`,
`数字设计原理与实践`,
`管理：使命、责任、实践（使命篇）`,
`兰海说成长 : 懂得孩子，才能更好帮助孩子`,
`開發精確的思考`,
`環境理性與制度抉擇`,
`民族进化的心理定律`,
`Linux嵌入式系统高级程序设计`,
`简素 : 日本文化的底色`,
`给孩子的世界地理大数据（全六册）`,
`公共管理论丛`,
`The World According to Star Wars`,
`Linux内核API完全参考手册`,
`架构之美：行业思想领袖揭秘软件设计之美（评注版）`
];
const REPEATED_BOOK_LIST_BAK = [
`计算机程序设计艺术・卷1 : 基本算法（第3版）`,
`科学革命的结构`,
`营销管理（第16版）`,
`代码大全2（纪念版） : 软件构建之道`,
`C++ Templates（第2版 英文版）`,
`鲍比如何说系列（《鲍比如何说 妈妈才会听》 《鲍比如何说 朋友才会听》） : 《鲍比如何说 妈妈才会听》 《鲍比如何说 朋友才会听》`,
`王羲之《兰亭序》`,
`丘吉尔：第二次世界大战回忆录 : 第一卷 铁血风暴`,
`几何原本`,
`深入理解计算机系统（原书第3版）`,
`资治通鉴`,
`计算机体系结构（英文版·第6版） : 量化研究方法`,
`艺术的故事`,
`米塞斯大传`,
`UNIX网络编程 卷2：进程间通信（第2版）`,
`冰与火之歌 : （1-5卷：权力的游戏、列王的纷争、冰雨的风暴、群鸦的盛宴、魔龙的`,
`工具论`,
`经济学原理(第7版) : 微观经济学分册+宏观经济学分册`,
`具体数学 : 计算机科学基础`,
`普林斯顿微积分读本(修订版)`,
`莎士比亚全集 : 增订本`,
`超标量处理器设计 : Superscalar Risc Processor Design`,
`爷爷变成了幽灵`,
`战争艺术史`,
`战争与和平`,
`汇编语言（第4版）`,
`魔戒`,
`孙子兵法`,
`写给儿童的世界历史 : （全16册）`,
`Effective C++ : 改善程序与设计的55个具体做法`,
`Unix & Linux大学教程`,
`安徒生童话全集`,
`丁丁历险记（共22本）`,
`费恩曼物理学讲义（第1卷） : (第1卷)`,
`费马大定理 : 一个困惑了世间智者358年的谜`,
`海底的秘密`,
`离散数学及其应用（原书第8版）`,
`贫嘴张大民的幸福生活`,
`三国演义`,
`传习录`,
`王阳明全集`,
`西方哲学史讲演录`,
`西游记（全二册）`,
`1984 : 反乌托邦小说三部曲`,
`一九八四`,
`银河帝国：基地七部曲 : 百万册纪念版`,
`最後的問題 : 西方短篇科幻小說精選`,
`Go程序设计语言(英文版)`,
`汇编语言与计算机体系结构`,
`积极改变：成就职场达人的情商课`,
`计算机科学概论（第12版）`,
`3-8岁儿童的数学经验`,
`数学经验`,
`精神现象学 : 黑格尔著作集·第3卷`,
`逻辑学Ⅰ : 黑格尔著作集·第5卷`,
`我们的身体`,
`Python 3破冰人工智能 从入门到实战 : 3破冰人工智能`,
`努门诺尔与中洲之未完的传说`,
`C程序设计语言（第2版·新版）`,
`CLR via C`,
`Effective Objective-C 2.0 : 52 Specific Ways to Improve Your iOS and OS X Programs`,
`UNIX环境高级编程 : 第2版`,
`魔鬼家书 : 地狱来鸿`,
`费曼物理学讲义 : 第1卷`,
`概率导论（第2版·修订版）`,
`供应链管理（第6版）`,
`计算机网络：系统方法（原书第5版） : 系统方法`,
`计算机网络（第8版）`,
`计算机网络（原书第7版） : 自顶向下方法`,
`你不知道的JavaScript`,
`特斯拉自传`,
`天朝的崩溃（修订版） : 鸦片战争再研究`,
`图解密码技术（第3版）`,
`昨日的世界 : 一个欧洲人的回忆`,
`组合数学（原书第5版）`,
`组织与管理`,
`计算机程序的构造和解释(原书第2版)`,
`计算机科学导论（原书第4版）`,
`毛泽东传（共6册）`,
`小熊宝宝绘本（全15册）`,
`写给儿童的中国历史`,
`中华人民共和国国民经济和社会发展第十三个五年规划纲要`,
`数据结构（C++语言版·第3版）`,
`教育科学与儿童心理学`,
`智慧的疆界 : 从图灵机到人工智能`,
`概率导论（第2版·修订版）`,
`DK艺术博物馆：世界名作全景导读`,
`百年孤独`,
`中国成语故事 : 图文本`,
`吹小号的天鹅`,
`春秋战国`,
`慈禧全传`,
`第三方JavaScript编程`,
`经济解释（二O一九增订版） : 五卷本`,
`矩阵计算`,
`亲密关系`,
`什么是数学 : 对思想和方法的基本研究`,
`淘气包埃米尔`,
`一站式学习C编程`,
`应用随机过程（第11版） : 概率模型导论`,
`Advanced Compiler Design and Implementation`,
`人工智能 : 一种现代的方法`,
`漫画算法：小灰的算法之旅（Python篇）`,
`数理逻辑`,
`数学分析八讲`,
`数值分析 : （原书第二版）`,
`算法设计`,
`所罗门王的指环`,
`郑渊洁童话全集(33卷)`,
`中国历代政治得失`,
`数学家谈怎样学数学`,
`动物庄园`,
`人工智能 : 一种现代的方法`,
`故事 : 材质·结构·风格和银幕剧作的原理`,
`入世哲学家 : 阿尔伯特·赫希曼的奥德赛之旅`,
`Head First 设计模式（中文版）`,
`Python数据科学手册`,
`Windows程序设计`,
`爱因斯坦传（精装珍藏版）`,
`白夜行`,
`传习录`,
`创新者的窘境 : 领先企业如何被新兴企业颠覆？`,
`聪明的投资者（第4版注疏点评版）`,
`方法论`,
`改变心理学的40项研究（第7版）`,
`哈姆雷特 : 莎士比亚悲剧·哈姆雷特`,
`建筑的永恒之道`,
`结构化计算机组成`,
`控制论 : 或动物与机器的控制和通信的科学`,
`昆虫Q&A`,
`皮特猫·第一辑（全6册）`,
`平面国 : 多维空间传奇往事`,
`如何说孩子才会听 怎么听孩子才肯说 : 中文五周年修订珍藏版`,
`丧钟为谁而鸣 : 丧钟为谁而鸣`,
`火速逃离平江路`,
`土鸡的冒险`,
`网络是怎样连接的`,
`微观经济学 : 现代观点（第九版）`,
`我的朋友安德烈`,
`狭义与广义相对论浅说`,
`小淘气尼古拉绝版故事`,
`新教伦理与资本主义精神`,
`心理学`,
`这才是心理学`,
`心理学与生活（第19版）`,
`这不是我的帽子`,
`组织的反思`,
`罗马人的故事（修订版，全15册）`,
`猫头鹰王国`,
`美的历程`,
`美国纽约摄影学院摄影教材（上册）`,
`美丽新世界`,
`深入Linux设备驱动程序内核机制`,
`数据压缩导论`,
`舒克和贝塔历险记`,
`数学天书中的证明`,
`自动机理论、语言和计算导论(英文版.第3版)`,
`分析与思考 : 黄奇帆的复旦经济课`,
`知行 : 技术人的管理之路`,
`线性代数`,
`局外人`,
`从历史中醒来 : 孙机谈中国古文物`,
`DOOM启世录`,
`Java 8实战`,
`JavaScript语言精粹`,
`Microsoft.NET框架程序设计`,
`澄衷蒙学堂字课图说`,
`褚时健传（修订版）`,
`创造现代世界 : 英国启蒙运动钩沉`,
`从一到无穷大 : 科学中的事实和臆测`,
`灯塔`,
`东京梦华录 : 全彩插图版`,
`高空走索人`,
`里外更新 : 生命的破碎与重建`,
`互联网人`,
`20世纪思想史 : 从弗洛伊德到互联网`,
`互联网思想十讲 : 北大讲义`,
`剑指Offer : 名企面试官精讲典型编程题`,
`江城`,
`历史的巨镜`,
`人间正道是沧桑`,
`社会性动物`,
`什么是数学 : 对思想和方法的基本研究`,
`天才引导的历程 : 数学中的伟大定理`,
`我变成一只喷火龙了`,
`小马过河`,
`信息论基础`,
`《科学美国人》趣味数学集锦之一`,
`隐藏的现实 : 平行宇宙是什么`,
`永恒的终结 : 关于时间旅行的终极奥秘和恢宏构想`,
`管理与组织研究常用的60个理论`,
`组织理论 : 理性、自然与开放系统的视角`,
`A Pattern Language : Towns, Buildings, Construction`,
`超越时空 : 通过平行宇宙、时间卷曲和第十维度的科学之旅`,
`城南旧事 : 纪念普及版`,
`程序设计实践`,
`华杉讲透《孙子兵法》（修订版）`,
`极简宇宙史`,
`领域驱动设计 : 软件核心复杂性应对之道`,
`论语`,
`毛姆短篇小说精选集`,
`剑指Offer : 名企面试官精讲典型编程题`,
`深入解析Windows操作系统`,
`深入浅出设计模式（影印版）`,
`数学的发现 : 对解题的理解、研究和讲授`,
`数学世纪 : 过去100年间30个重大问题`,
`数学与猜想（第一卷） : 数学中的归纳和类比`,
`数字集成电路 : 电路、系统与设计`,
`追寻记忆的痕迹 : 新心智科学的开创历程`,
`自然哲学之数学原理`,
`爱上阅读的秘密2`,
`程序员修炼之道（第2版） : 通向务实的最高境界`,
`领域驱动设计 : 软件核心复杂性应对之道`,
`所有我们看不见的光`,
`1453 : 君士坦丁堡之战`,
`众病之王 : 癌症传`,
`天才的编辑 : 麦克斯·珀金斯与一个文学时代`,
`The Daily Show with Jon Stewart Presents Earth : A Visitor's Guide to the Human Race`,
`DARK SOULS 官方艺术设定集Ⅰ&Ⅱ`,
`iOS编程实战`,
`Java安全编码标准`,
`Java并发编程实战`,
`Java核心技术·卷 I（原书第11版） : 基础知识`,
`Linux C编程一站式学习`,
`Linux内核完全剖析 : 基于0.12内核`,
`Linux命令行与shell脚本编程大全（第3版）`,
`UNIX编程艺术`,
`爱与黑暗的故事`,
`编程精粹 : 编写高质量C语言代码`,
`别闹了，费曼先生 : 科学顽童的故事`,
`操作系统 : 精髓与设计原理（原书第6版）`,
`从一到无穷大`,
`代数特征值问题`,
`理查德·费曼传`,
`概率论及其应用（卷1•第3版）`,
`高性能JavaScript`,
`格林童话全集`,
`格鲁夫给经理人的第一课 : 英特尔创始人自述`,
`管理的实践`,
`集体智慧编程`,
`巨人三传 : 傅译名著系列`,
`卡尔•威特的教育`,
`科学管理原理`,
`酷虫学校1 : 酷虫学校`,
`快乐儿童的7个习惯`,
`离散数学与组合数学`,
`历史绪论`,
`历史研究（上下卷）`,
`让孩子心悦诚服 : 亲子沟通的技巧和训练`,
`人间词话`,
`深度探索C++对象模型`,
`万历十五年`,
`现代体系结构的优化编译器`,
`嫌疑人X的献身`,
`唐山 : 一个世纪的故事`,
`应用密码学 : 协议、算法与C源程序`,
`幽梦影`,
`与孩子共享自然`,
`战国策（全二册）`,
`战争论（全三卷）`,
`宗教经验之种种`,
`组织行为学(第16版)`,
`写作这回事 : 创作生涯回忆录`,
`论中国`,
`南怀瑾选集（第一卷） : 论语别裁`,
`深入Linux内核架构`,
`深度探索C++对象模型`,
`数据结构与算法分析 : C语言描述`,
`数据结构与算法分析 : C++描述`,
`数据结构与算法分析 : Java语言描述（英文版·第3版）`,
`数学女孩2 : 费马大定理`,
`数字逻辑应用与设计`,
`红楼梦`,
`水浒传`,
`说服力`,
`说服力：工作型PPT该这样做`,
`世界观（原书第3版） : 现代人必须要懂的科学哲学和科学史`,
`人的行为`,
`组织行为学(第16版)`,
`进化心理学(第4版) : 心理的新科学`,
`科学革命的结构`,
`我们赖以生存的隐喻`,
`制度、制度变迁与经济绩效`,
`亲密关系（第5版）`,
`结构性改革 : 中国经济的问题与对策`,
`给孩子的人工智能通识课`,
`斯通纳`,
`阿兰的战争 : 全三册`,
`微不足道的生活`,
`二手时间`,
`深泽直人`,
`东京本屋`,
`零年 : 1945·现代世界诞生的时刻`,
`Effective Java 中文版`,
`HR劳动争议经典管理案例`,
`Intel汇编语言程序设计 : Intel汇编语言程序设计`,
`JavaScript Patterns`,
`百万富翁的智慧`,
`长日将尽`,
`肠子的小心思`,
`打开绘本之眼`,
`第一本经济学 : 升级版`,
`疯狂星期二`,
`复杂SOC设计`,
`高性能网站建设进阶指南 : Web开发者性能优化最佳实践`,
`管理学(第13版) : 管理学`,
`计算机体系结构 : 计算机体系结构嵌入式方法`,
`计算机体系结构 : 量化研究方法（第5版）`,
`计算机系统要素 : 从零开始构建现代计算机`,
`基因组 : 人种自传23章`,
`金融时间序列分析`,
`金色梦乡`,
`三言·警世通言 : 三言·警世通言`,
`精通正则表达式 : 第3版`,
`竞争论`,
`了不起的我 : 自我发展的心理学`,
`历代经济变革得失（典藏版）`,
`皮皮鲁传`,
`强力与弱力 : 破解宇宙深层的隐匿魔法`,
`认知心理学及其启示`,
`统计学习理论的本质`,
`自然哲学的数学原理`,
`图解设计模式`,
`我的图画书论`,
`现代编译原理 : C语言描述（修订版）`,
`现代操作系统（原书第4版）`,
`想赢的男孩`,
`艺术简史`,
`与“众”不同的心理学 : 如何正视心理学（第七版）`,
`月下看猫头鹰`,
`战略管理(概念与案例第12版)/工商管理经典译丛`,
`零年 : 1945·现代世界诞生的时刻`,
`The Master Algorithm : How the Quest for the Ultimate Learning Machine Will Remake Our`,
`明解C语言（第3版） : 入门篇`,
`沈石溪动物小说自选集`,
`邓小平时代`,
`算法：C语言实现 : (第1～4部分)基础知识、数据结构、排序及搜索`,
`数学家讲解小学数学`,
`自然哲学之数学原理`,
`数学家谈怎样学数学`,
`古今数学思想（一）`,
`数学女孩`,
`数学史（上下）`,
`社会心理学（第8版） : 阿伦森眼中的社会性动物`,
`硅谷增长黑客实战笔记`,
`冷血`,
`自体心理学的理论与实践 : 万千心理`,
`行动的勇气 : 金融危机及其余波回忆录`,
`山中小记：圆霖长老随缘开示`,
`美国的尤利西斯 : 尤利西斯·S.格兰特的故事`,
`Effective Java中文版（第3版）`,
`Effective STL中文版 : 50条有效使用STL的经验`,
`Fantastic Man`,
`MIPS体系结构透视`,
`Orange'S:一个操作系统的实现`,
`SRE : Google运维解密`,
`Web性能权威指南`,
`Windows核心编程`,
`wireshark网络分析的艺术`,
`奥丽芙·基特里奇`,
`编程之美 : 微软技术面试心得`,
`别以为你懂孩子的心`,
`软件架构设计 : 大型网站技术架构与业务架构融合之道`,
`滴血的龙-超级成长版冒险小虎队 : 滴血的龙`,
`动机心理学`,
`独裁者手册 : 为什么坏行为几乎总是好政治`,
`仿生人会梦见电子羊吗？`,
`滚雪球：巴菲特和他的财富人生·上（畅销版）`,
`孩子，把你的手给我 : 与孩子实现真正有效沟通的方法`,
`精益创业实战（第2版）`,
`让数字说话 : 审计，就这么简单`,
`矩阵分析`,
`看见`,
`量子宇宙`,
`鸟哥的Linux私房菜 : —服务器架设篇(第三版)`,
`苏菲的世界`,
`人际关系心理学`,
`荣耀与丑闻 : 反思德国浪漫主义`,
`如何说孩子才会听，怎么听孩子才肯说`,
`如何说孩子才肯学`,
`如何说孩子才能和平相处`,
`我的图画书论`,
`线性代数应该这样学 : 第3版`,
`写在人生边上`,
`新天下之化 : 明初礼俗改革研究`,
`幸福的种子 : 亲子共读图画书`,
`星际穿越`,
`牙齿大街的新鲜事 : 德国精选科学图画书`,
`银河系漫游指南`,
`月亮与六便士`,
`如何说孩子才会听，怎么听孩子才肯说`,
`债务危机 : 我的应对原则`,
`最美最美的中国童话 : 台湾汉声❤世纪经典版本`,
`做个不骂孩子的妈妈 : 亲子语言沟通对错280句`,
`About Face 3：交互设计精髓`,
`程序设计方法（第2版）`,
`汇编语言程序设计`,
`货币金融学（第十二版）`,
`明史（全二十八册）`,
`时间的形状 : 相对论史话（升级版）`,
`数学简史 : 确定性的消失`,
`算法问题实战策略`,
`猪八戒吃西瓜`,
`宗教经验种种 : 人性的研究`,
`世界图画书阅读与经典`,
`十万个为什么`,
`发展心理学：儿童与青少年 : 第9版（万千心理）`,
`博弈与社会`,
`血疫 : 埃博拉的故事`,
`僧侣与哲学家`,
`骄傲之塔 : 战前世界的肖像，1890-1914`,
`C算法（第二卷：图算法）（第3版）`,
`HTML5权威指南`,
`Linux内核源代码情景分析（上册）`,
`Linux设备驱动程序`,
`MySQL运维内参 : MySQL、Galera、Inception核心原理与最佳实践`,
`Python源码剖析 : 深度探索动态语言核心技术`,
`Redis设计与实现`,
`史蒂夫·乔布斯传`,
`STL源码剖析`,
`程序员的职业素养`,
`Web界面设计`,
`Linux内核源代码情景分析（上册）`,
`Linux内核源代码情景分析（上册）`,
`安德的游戏·三部曲 : 全新典藏版`,
`巴菲特之道（原书第3版）（典藏版）`,
`百歲老人蹺家去`,
`贝多芬传`,
`编写可读代码的艺术`,
`病者生存：疾病如何延续人类寿命 : 疾病如何延续人类寿命`,
`曾国藩大传`,
`曾国藩（唐浩明讲解版）`,
`曾国藩全集（全叁拾壹册） : 全31册`,
`曾国藩家书`,
`曾国藩家书`,
`程序员的职业素养`,
`程序员修炼之道 : 从小工到专家`,
`创新公司 : 皮克斯的启示`,
`大败局Ⅱ`,
`大秦帝国(套装全6部11卷)`,
`大学的理念`,
`带一本书去巴黎`,
`点线面`,
`恶意`,
`凤凰项目 : 一个IT运维的传奇故事`,
`高级编译器设计与实现`,
`梦想与浮沉 : A股十年上市博弈`,
`管理行为`,
`光荣与梦想（套装共4册） : 1932-1972年美国社会实录`,
`黑客攻防技术宝典（第2版） : Web实战篇（第2版）`,
`猴子捞月`,
`架构整洁之道`,
`间客`,
`孔子传`,
`困境与出路 : 企业如何制定破坏性增长战略`,
`纽约客`,
`旁观者 : 管理大师德鲁克回忆录`,
`软件随想录 : 程序员部落酋长Joel谈软件`,
`设计的教室`,
`深入Java虚拟机(原书第2版)`,
`态度改变与社会影响`,
`挑战程序设计竞赛`,
`投资中最简单的事（更新版）`,
`网络体系结构及设计`,
`微积分的历程 : 从牛顿到勒贝格`,
`5分钟商学院（套装）`,
`性能之巅（第2版）：系统、企业与云可观测性`,
`用户体验度量：收集、分析与呈现（第2版）（全彩）`,
`怎样解题 : 数学思维的新方法`,
`C++沉思录`,
`计算广告 第2版 : 互联网商业变现的市场与技术`,
`旅途上的建筑 : 漫步欧洲`,
`罗伯特议事规则（第11版） : Robert's Rules of Order`,
`罗生门`,
`蒙台梭利教育法`,
`深入理解LINUX内核(第三版)`,
`生命中最重要的 : 如何将个人和组织的价值发挥到极致`,
`史蒂夫·乔布斯传`,
`谢辰生口述 : 新中国文物事业重大决策纪事`,
`正念禅修 : 在喧嚣的世界中获取安宁`,
`中國字體設計人：一字一生`,
`鳄鱼街 : 布鲁诺·舒尔茨小说全集`,
`索拉里斯星`,
`3D打印 : 三维智能数字化创造`,
`逃避统治的艺术（修订译本） : 东南亚高地的无政府主义历史`,
`另一个世界 : 中国记忆1961-1962`,
`The Clean Coder : A Code of Conduct for Professional Programmers`,
`Conscious Business`,
`CSS设计彻底研究`,
`JavaScript DOM编程艺术 （第2版）`,
`EECS应用概率论`,
`HTTP权威指南 : 权威指南`,
`JavaScript DOM编程艺术 （第2版）`,
`Linux 系统编程（第二版）`,
`一切行业都是创意业 : 我创立维珍商业帝国的故事`,
`Objective-C高级编程 : iOS与OS X多线程和内存管理`,
`Oracle JRockit : The Definitive Guide`,
`第三种黑猩猩：人类的身世与未来（简明版）`,
`Web 2.0界面设计模式`,
`Wireshark网络分析就这么简单`,
`爱上电子学 : 创客的趣味电子实验`,
`本杰明·富兰克林自传`,
`编程原本`,
`编码宝典`,
`Web表单设计 : 点石成金的艺术`,
`持续交付 : 发布可靠软件的系统方法`,
`大国的兴衰 : 1500-2000年的经济变迁与军事冲突`,
`第56号教室的奇迹 : 让孩子变成爱学习的天使`,
`第五项修炼 : 学习型组织的艺术与实践`,
`泛型编程与STL`,
`高效程序的奥秘`,
`高性能网站建设指南 : 前端工程师技能精髓`,
`个体化`,
`给忙碌者的天体物理学`,
`海底两万里`,
`黑天鵝效應`,
`About Face 4: 交互设计精髓`,
`界面设计模式（第2版）`,
`精通JavaScript`,
`垃圾回收的算法与实现`,
`朗读手册`,
`了凡四训`,
`你干吗在乎别人怎么想？ : 充满好奇心的费曼`,
`平行宇宙`,
`清史稿(全四十八册)`,
`穷查理宝典 : 查理•芒格智慧箴言录`,
`如何求解问题 : 现代启发式方法`,
`商战（经典重译版）`,
`设计中的设计`,
`伟大的博弈 : 华尔街金融帝国的崛起（1653-2011）`,
`文明的冲突与世界秩序的重建`,
`我的兔子朋友`,
`性心理学`,
`小鸡的一千个秘密 : 旁帝经典·面具小鸡布莱兹系列`,
`怎样给孩子讲故事`,
`Android 源码设计模式解析与实战`,
`法治及其本土资源`,
`漫步华尔街`,
`富爸爸财务自由之路`,
`财富自由之路`,
`深入浅出Node.js`,
`世界儿童文学·阅读与经典 : 世界儿童文学阅读与经典`,
`隨機騙局──潛藏在生活與市場中的機率陷阱`,
`写给大家看的设计书（第3版）`,
`写给儿童的中国地理 : （全14册）`,
`证明与反驳 : 数学发现的逻辑`,
`日常生活中的自我呈现`,
`文化的解释`,
`儿童教育心理学 : 儿童的人格形成及其培养`,
`陶哲轩教你学数学`,
`历代经济变革得失`,
`人生的起点和终点`,
`0-6岁A+育儿法`,
`混序：维萨与组织的未来形态 : 维萨与组织的未来形态`,
`我脑袋里的怪东西`,
`子不语 : 全新点校本`,
`走向新建筑`,
`中国历史研究手册`,
`JavaScript DOM高级程序设计`,
`JAVASCRIPT语言精髓与编程实践`,
`Java解惑`,
`Linux内核设计与实现(原书第3版)`,
`MATLAB数值计算`,
`深入理解Nginx : 模块开发与架构解析`,
`OpenGL编程指南`,
`ppk谈JavaScript`,
`编程人生 : 15位软件先驱访谈录`,
`编程珠玑`,
`编译器设计（第2版）`,
`创新者：一群技术狂人和鬼才程序员如何改变世界 : 一群技术狂人和鬼才程序员如何改变世界`,
`发现的乐趣 : 费曼演讲、访谈集`,
`费曼手札 : 不休止的鼓声`,
`真正的蒙氏教育在家庭 : 50个经典数学游戏造就孩子思维力`,
`哈佛通识教育红皮书`,
`政治与行政 : 政府之研究`,
`结构化研讨 : 参与式决策操作手册`,
`解忧杂货店`,
`看人 : 肢体语言导读`,
`昆虫脸书`,
`莱博维茨的赞歌`,
`老舍作品选读`,
`鸟哥的Linux私房菜 基础学习篇 第四版`,
`启示录 : 打造用户喜爱的产品`,
`七周七语言 : 理解多种编程范型`,
`趣味学数学`,
`人工智能 : 人工智能·智能系统指南（原书第2版）`,
`人生`,
`人性中的善良天使 : 暴力为什么会减少`,
`通灵芯片 : 计算机运作的简单原理`,
`童年的秘密`,
`UNIX系统编程: 通信、并发与线程`,
`投资者的未来（典藏版）`,
`图解密码技术`,
`图解HTTP`,
`图解设计模式`,
`哇，历史原来可以这样学1`,
`为什么读经典`,
`我们为什么会生病`,
`系統思考 : 克服盲點.面對複雜性.見樹又見林的整體思考`,
`新经济，新规则`,
`有限性之后 : 论偶然性的必然性`,
`战略品牌管理`,
`梦醒子 : 一位华北乡居者的人生`,
`哈耶克文选 : 哈耶克论文演讲集`,
`哈内克论哈内克`,
`论个人在历史上的作用问题`,
`面向模式的软件架构 卷5`,
`算法设计与分析基础(第3版) : 第3版`,
`拯救日本 : 泡沫崩溃后的银行危机与华尔街行动`,
`中国国家地理（选美中国）`,
`自驱型成长：如何科学有效培养孩子的自律`,
`十问 : 霍金沉思录`,
`平和式教养法（多子女篇） : 让孩子停止纷争，保持亲密情谊`,
`现代制度经济学(上下)`,
`最长的一天 : 1944诺曼底登陆`,
`纳粹医生 : 医学屠杀与种族灭绝心理学`,
`《华尔街日报》是如何讲故事的`,
`51種物戀`,
`轴心时代 : 人类伟大思想传统的开端`,
`代码整洁之道`,
`Don't Make Me Think : A Common Sense Approach to Web Usability`,
`Java虚拟机规范（Java SE 8版） : Java虚拟机规范`,
`MySQL技术内幕 : InnoDB存储引擎(第2版)`,
`企业应用架构模式`,
`S.`,
`Swifter : 100 个 Swift 开发必备 Tip`,
`来自12个星球的敌人 : 垂暮之战`,
`Web性能实践日志`,
`把时间当作朋友（第3版）`,
`印度：百万叛变的今天 : India: A Million Mutinies Now`,
`操作系统概念（原书第9版）`,
`创造突破性产品 : 从产品策略到项目定案的创新`,
`大教堂与集市`,
`DK大历史 : 从宇宙大爆炸到我们人类的未来，138亿年的非凡旅程`,
`大历史 : 虚无与万物之间`,
`代码整洁之道`,
`点石成金 : 访客至上的网页设计秘笈`,
`度量 : 一首献给数学的情歌`,
`对冲基金风云录1 : 东邪西毒`,
`多处理器编程的艺术（原书第2版）`,
`沸腾十五年：中国互联网1995—2009（修订版）`,
`福布斯 Forbes`,
`父母的觉醒`,
`哥伦布大交换 : 1492年以后的生物影响和文化冲击`,
`国境以南 太阳以西`,
`合作的进化`,
`简约至上 : 交互式设计四策略（第2版）`,
`经济学的思维方式`,
`精通CSS : 高级Web标准解决方案`,
`恐龙文明三部曲 : 远望、化石猎人、异族`,
`两次全球大危机的比较研究`,
`企业应用架构模式`,
`全球科技通史`,
`如何赢得朋友及影响他人`,
`少有人走的路 : 心智成熟的旅程`,
`身份的焦虑`,
`淘气的阿柑`,
`文明与野蛮`,
`无线电安全攻防大揭秘`,
`消费者行为学（第12版）（工商管理经典译丛·市场营销系列）`,
`学习乐观`,
`寻欢作乐`,
`演员的自我修养`,
`耶路撒冷三千年`,
`飲膳札記`,
`游戏设计的236个技巧 : 游戏机制、关卡设计和镜头窍门`,
`游戏玩家`,
`语言本能 : 人类语言进化的奥秘`,
`怎样卖龙虾`,
`好奇宝贝科学思维系列绘本：这是谁的牙齿`,
`《华尔街日报》是如何讲故事的`,
`计算复杂性理论基础`,
`跟我们做流程管理`,
`驴小弟变石头`,
`罗素传 : 孤独的精神 1872—1920`,
`米开朗琪罗传`,
`面向模式的软件体系结构 卷2:用于并发和网络化对象的模式`,
`明天要遠足`,
`深入理解Android内核设计思想`,
`深入理解C指针`,
`数学与生活（修订版）`,
`数字设计和计算机体系结构（原书第2版）`,
`搜索引擎 : 信息检索实践`,
`算法设计与分析基础`,
`行为自动机研究 : 选择性综合神经模拟`,
`沙丘六部曲`,
`超越CSS : Web设计艺术精髓`,
`中毒的父母`,
`整体养育`,
`人人文化： 锐意发展型组织DDO`,
`策略思维 : 商界、政界及日常生活中的策略竞争`,
`华为访谈录 : 一部华为历史故事辞典`,
`中國字體設計人：一字一生`,
`不平等的代价（珍藏版）`,
`30天自制操作系统`,
`脑机穿越 : 脑机接口改变人类未来`,
`无穷的开始 : 世界进步的本源`,
`美国裂变 : 大历史转折点上的总统大选`,
`那年夏天 : 美国1927`,
`忍不住的“关怀” : 1949年前后的书生与政治`,
`C专家编程`,
`Debug Hacks中文版 : 深入调试的技术和工具`,
`大话设计模式`,
`J2EE设计开发编程指南`,
`Linux内核源代码情景分析（下册）`,
`Modern Compiler Implementation in C`,
`寒江独钓 : Windows内核安全编程`,
`x86/x64体系探索及编程`,
`爱哭鬼小隼`,
`艾伦·图灵传 : 如谜的解谜者`,
`暗时间（第2版） : 思维改变生活`,
`白领 : 美国的中产阶级`,
`宝葫芦的秘密`,
`北野武的伟大旅程 : 重新发现世界的十一次探险`,
`编程匠艺 : 编写卓越的代码`,
`编程卓越之道 : 第二卷：运用底层语言思想编写高级语言代码`,
`长安十二时辰 上`,
`程序员的思维修炼 : 开发认知潜能的九堂课`,
`创业维艰 : 如何完成比难更难的事`,
`從大歷史角度讀蔣介石日記`,
`“错误”的行为 : 行为经济学的形成`,
`大败局`,
`大败局Ⅰ`,
`大空头`,
`大明王朝的七张面孔`,
`大数据日知录 : 架构与算法`,
`地球纪元`,
`搞定 : 无压工作的艺术`,
`搞定 : 无压工作的艺术`,
`关键在问 : 焦点讨论法在学校中的应用`,
`管理决策新科学（国外经济管理名著丛书）`,
`硅谷革命 : 成就苹果公司的疯狂往事`,
`硅谷热`,
`黑客 : 计算机革命的英雄`,
`基业长青 : 企业永续经营的准则`,
`简捷启发式 : 让我们更精明`,
`建筑中的数学之旅`,
`世界历史的教训 : 民族国家信仰及其祸福`,
`历史的教训`,
`企业生命周期`,
`晴朗的一天 : 启发精选凯迪克大奖绘本`,
`清日战争 : 1894—1895`,
`富爸爸，穷爸爸`,
`人生忽如寄 : 跟梁实秋品味雅致人生`,
`如何阅读一本书`,
`三双鞋 : 美捷步总裁谢家华自述`,
`我的职业是小说家`,
`我家有个小学生`,
`我们属龙`,
`无懈可击的Web设计II`,
`无言的宇宙 : 隐藏在24个数学公式背后的故事`,
`系统之美 : 决策者的系统思考`,
`新参者`,
`新异化的诞生 : 社会加速批判理论大纲`,
`研究之美`,
`野蛮生长`,
`一个定理的诞生 : 我与菲尔茨奖的一千个日夜`,
`引导 : 团队群策群力的实践指南`,
`用户体验的要素 : 以用户为中心的Web设计`,
`用户体验面面观 : 方法、工具与实践`,
`用户体验要素 : 以用户为中心的产品设计`,
`再会，老北京 : 一座转型的城，一段正在消逝的老街生活`,
`掌握需求过程(第3版)`,
`找寻逝去的自我 : 大脑、心灵和往事的记忆`,
`超弦理论 : 探究时间、空间及宇宙的本原`,
`成为技术领导者 : 解决问题的有机方法`,
`激荡十年，水大鱼大 : 中国企业2008—2018`,
`领导梯队 : 全面打造领导力驱动型公司(原书第2版)`,
`鲁宾逊漂流记`,
`美国陷阱 : 如何通过非商业手段瓦解他国商业巨头`,
`米小圈上学记 : 我有一个跟屁虫`,
`智能时代 : 大数据与智能革命重新定义未来`,
`能量：9个能量场实验激活不可思议人生 : 9个能量场实验激活不可思议人生`,
`你必须知道的495个C语言问题`,
`深入理解LINUX网络技术内幕`,
`生命3.0 : 人工智能时代，人类的进化与重生`,
`胜于言传 : 网站内容制胜宝典`,
`时间回旋`,
`数据之巅 : 大数据革命,历史、现实与未来`,
`思维模型`,
`思维与决策（第四版）`,
`鞋狗 : 耐克创始人菲尔·奈特亲笔自传`,
`重新定义公司 : 谷歌是如何运营的`,
`世界秩序`,
`领导梯队 : 全面打造领导力驱动型公司(原书第2版)`,
`原生家庭 : 如何修补自己的性格缺陷`,
`管理的常识 : 让管理发挥绩效的8个基本概念(修订版)`,
`善战者说 : 孙子兵法与取胜法则十二讲`,
`人工智能 : 一种现代的方法`,
`1968 : 撞击世界之年`,
`2018`,
`架构即未来：现代企业可扩展的Web架构、流程和组织(原书第2版)`,
`选择流落`,
`一定要给你个惊喜 : 谈波短篇小说选`,
`安尼尔的鬼魂`,
`The Undoing Project : A Friendship That Changed Our Minds`,
`制造大片 : 金钱、权力与好莱坞的秘密`,
`水的答案知多少 : 水的第四相：不只是固态、液态和气态`,
`那些古怪又让人忧心的问题 : What If`,
`裸城 : 原真性城市场所的生与死`,
`千面英雄`,
`编年史`,
`CSS商业网站布局之道 : 一本Web标准的新书`,
`Docker——容器与容器云（第2版）`,
`Java Collections`,
`Java性能优化权威指南 : Java性能优化权威指南`,
`Linux内核完全注释`,
`Linux设备驱动开发详解`,
`Linux那些事儿之我是USB`,
`爱和自由`,
`爱丽丝漫游奇境记`,
`白帽子讲Web安全`,
`编写可维护的JavaScript`,
`并行程序设计原理`,
`波希米亚 : 迷人的放逐`,
`菜鸟侦探挑战数据分析`,
`禅与摩托车维修艺术`,
`程序员的数学3 : 线性代数`,
`从优秀到卓越`,
`促动 : 激活团队能量的新领导力法则`,
`点石成金 : 访客至上的Web和移动可用性设计秘笈`,
`东京大学通识讲座3 : 纯情篇`,
`富甲美国 : 零售大王沃尔顿自传`,
`扶桑`,
`管理3.0 : 培养和提升敏捷领导力`,
`好战略，坏战略`,
`精益数据分析`,
`决策与判断`,
`决胜B端 : 产品经理升级之路`,
`狼图腾`,
`你一定爱读的极简欧洲史`,
`纽约时报100年`,
`新生——七年就是一辈子 : 七年就是一辈子`,
`奇特的一生`,
`企业集成模式 : 设计、构建及部署消息传递解决方案`,
`清教徒的礼物`,
`情色昆虫记 : 昆虫世界的爱情兵法`,
`穷理查年鉴 : 财富之路`,
`人类群星闪耀时 : 十四篇历史特写`,
`設計師的材料學 : 創意×實驗×未來性，從原始材料到創新材質的112個設計革命`,
`态度 : 吴军家书`,
`系统化思维导论`,
`现在可以说了 : 犹太智者基翁幽默文选`,
`心流 : 最优体验心理学`,
`幸福的方法 : 哈佛大学最受欢迎的幸福课`,
`引力是什么 : 支配宇宙万物的神秘之力`,
`赢 : 韦尔奇一生的管理智慧`,
`蝇王`,
`游戏之旅 : 我的编程感悟`,
`在路上`,
`细听鬼唱诗 : 志异小品赏读`,
`华丽人生`,
`怀孕圣经（最终定本）`,
`计算机的心智 : 操作系统之哲学原理`,
`裸城 : 原真性城市场所的生与死`,
`美国创新史 : 从蒸汽机到搜索引擎，美国两个世纪历史上最著名的53位革新者`,
`梦的解析`,
`迷茫的旅行商 : 一个无处不在的计算机算法问题`,
`谜米机器`,
`敏捷开发的艺术`,
`深入解析Mac OS X & iOS操作系统`,
`深入理解JavaScript`,
`深入理解计算机网络`,
`数学悖论与三次数学危机`,
`思辨与立场 : 生活中无处不在的批判性思维工具`,
`思考的乐趣 : Matrix67数学笔记`,
`思维规律的研究`,
`支撑处理器的技术 : 永无止境地追求速度的世界`,
`智识分子 : 做个复杂的现代人`,
`重来 : 更为简单有效的商业思维`,
`精益数据分析`,
`幕后产品 : 打造突破式产品思维`,
`系统化思维导论`,
`数据挖掘导论`,
`华为管理变革`,
`华为成长之路 : 影响华为的22个关键事件`,
`一个人的朝圣`,
`漫长的中场休息`,
`Mindset : The New Psychology of Success`,
`20世纪最伟大的心理学实验`,
`城中城 : 社会学家的街头发现`,
`伍迪·艾伦谈话录`,
`CSS网站布局实录（第2版） : 基于Web标准的网站设计指南`,
`安德的游戏 : 全新典藏版`,
`Go语言学习笔记`,
`重新定义公司 : 谷歌是如何运营的`,
`Groovy程序设计`,
`Java性能权威指南`,
`Java编程语言`,
`Linux程序设计 : 第4版`,
`机器学习实战`,
`Web信息架构(第3版) : 设计大型网站`,
`Web导航设计`,
`巴西：未来之国 : 未来之国`,
`别做正常的傻瓜`,
`程序是怎样跑起来的`,
`程序员的数学（第2版）`,
`程序员的数学2 : 概率统计`,
`城中城 : 社会学家的街头发现`,
`赤裸裸的统计学 : 除去大数据的枯燥外衣,呈现真实的数字之美`,
`东京大学通识讲座：贯通篇`,
`富爸爸财务自由之路`,
`改善 : 日本企业成功的奥秘`,
`高效程序员的45个习惯 : 敏捷开发修炼之道`,
`黑客大曝光 : Web应用安全机密与解决方案（第2版）`,
`解析极限编程 : 拥抱变化`,
`今日简史 : 人类命运大议题`,
`精益软件开发管理之道`,
`九型人格 : 了解自我.洞悉他人的秘诀`,
`可用性工程`,
`你的生命有什么可能`,
`七周七语言 : 理解多种编程范型`,
`权力源自地位 : 北京大学、知识分子与中国政治文化，1898～1929`,
`人才管理圣经 : 人才管理圣经`,
`神经漫游者`,
`统计陷阱`,
`图解HTTP`,
`万万没想到 : 用理工科思维理解世界`,
`微服务架构设计模式`,
`微服务设计`,
`现代设计史（第2版）`,
`向前一步 : 女性，工作及领导意志`,
`新家庭如何塑造人（第二版）`,
`心智探奇 : 人类心智的起源与进化`,
`修改代码的艺术`,
`雪崩`,
`学会跟孩子说话`,
`学历无用论`,
`严厉的月亮`,
`一个广告人的自白（纪念版）`,
`用户体验草图设计 : 正确地设计,设计得正确`,
`幽暗国度 : 印度三部曲Ⅰ`,
`嗨，好久不见`,
`超越智商 : 为什么聪明人也会做蠢事`,
`寂静的春天`,
`领导者准则`,
`零售的哲学 : 7-Eleven便利店创始人自述`,
`麦田里的守望者`,
`秘密`,
`机器学习实战`,
`明日边缘`,
`奈飞文化手册`,
`谁说大象不能跳舞？`,
`思考，快与慢`,
`写给大忙人看的Java SE 8`,
`郑玉巧育儿经·婴儿卷`,
`第一性－女人的天賦正在改變世界`,
`重新定义公司 : 谷歌是如何运营的`,
`卓有成效的程序员`,
`自制编程语言`,
`沟通的艺术 : 看入人里，看出人外（插图第12版）`,
`数据分析思维 : 分析方法和业务知识`,
`习惯的力量 : 我们为什么会这样生活，那样工作？`,
`The Hidden Wealth of Nations`,
`知识与权力 : 科学的世界之旅`,
`22条商规 : 美国CEO最怕竞争对手读到的商界奇书`,
`生命的未来 : 从双螺旋到合成生命`,
`360度评估反馈法：人才管理的关键技术`,
`两访中国茶乡`,
`硅谷钢铁侠 : 埃隆·马斯克的冒险人生`,
`杜尚`,
`CSS那些事儿 : 掌握网页样式与CSS布局核心技术`,
`C算法(第一卷:基础、数据结构、排序和搜索)(第三版)`,
`硅谷钢铁侠 : 埃隆·马斯克的冒险人生`,
`HR转型突破 : 跳出专业深井成为业务伙伴`,
`Linux Shell脚本攻略`,
`异类 : 不一样的成功启示录`,
`长尾理论`,
`程序设计语言原理`,
`大规模Web服务开发技术`,
`代码审计 : 企业级Web代码安全架构`,
`代码之髓 : 编程语言核心概念`,
`德鲁克论管理`,
`第二曲线创新（第2版）`,
`颠覆完美软件 : 软件测试必须知道的几件事`,
`风格的要素：简明英语写作圣经（汉英对照）(附赠书)`,
`《父母最艰巨的工作》 : 帮助家庭迎接前所未遇的挑战`,
`构建可扩展的Web站点`,
`硅谷钢铁侠 : 埃隆·马斯克的冒险人生`,
`硅谷之火 : 个人计算机的诞生与衰落（第3版）`,
`机械宇宙 : 艾萨克·牛顿、皇家学会与现代世界的诞生`,
`见识`,
`金陵刻经处研究`,
`锦绣蓝图 : 怎样规划令人流连忘返的网站(第2版)`,
`理性乐观派 : 一部人类经济进步史`,
`链接器和加载器`,
`你要如何衡量你的人生`,
`奇思妙想 : 15位计算机天才及其重大发现`,
`情感化设计`,
`金钱的社会意义 : 私房钱、工资、救济金等货币`,
`童年的消逝`,
`推背圖`,
`我的第一本编程书`,
`习惯的力量 : 我们为什么会这样生活，那样工作？`,
`心理定向与成功`,
`幸福之源`,
`摇摆`,
`异类 : 不一样的成功启示录`,
`赢在用户 : Web人物角色创建和应用实践指`,
`宇宙最初三分钟 : 关于宇宙起源的现代观点`,
`这就是搜索引擎 : 核心技术详解`,
`最好的抉择 : 关于看病就医你要知道的常识`,
`Android软件安全与逆向分析`,
`Android软件安全与逆向分析`,
`反脆弱 : 从不确定性中获益`,
`流星之绊`,
`面向计算机科学的数理逻辑系统建模与推理`,
`魔鬼经济学系列`,
`那些古怪又让人忧心的问题 : 前NASA成员、美国最火科普博客xkcd幽默问答集`,
`你不可不知的50个数学知识`,
`深入理解Android : 卷I`,
`身为职场女性 : 女性事业进阶与领导力提升`,
`数值方法和MATLAB实现与应用`,
`瞬间之美 : Web界面设计如何让用户心动`,
`写出我心 : 普通人如何通过写作表达自己`,
`支付战争 : 互联网金融创世纪`,
`重新理解创业 : 一个创业者的途中思考`,
`自己动手写操作系统`,
`从一到无穷大 : 科学中的事实与猜测`,
`事实 : 用数据思考，避免情绪化决策`,
`实战Java高并发程序设计`,
`印度：受伤的文明 : India: A Wounded Civilization`,
`长尾理论`,
`人人都是产品经理2.0 : 写给泛产品经理`,
`消费者行为学 : 行动影响力`,
`硅谷之火 : 个人计算机的诞生与衰落（第3版）`,
`Autumn`,
`王国与权力 : 震撼世界的《纽约时报》`,
`负建筑`,
`增长的本质 : 秩序的进化，从原子到经济`,
`街道文字 : 在世界的街角，發現文字的秘密`,
`Facebook效应 : 看Facebook如何打造无与伦比的社交帝国`,
`Java虚拟机精讲`,
`LINUX与UNIX Shell编程指南`,
`编译器构造C语言描述`,
`大型网站技术架构 : 核心原理与案例分析`,
`定位 : 有史以来对美国营销影响最大的观念`,
`疯狂Java讲义`,
`规模 : 复杂世界的简单法则`,
`黑帮老大的一天`,
`规模 : 复杂世界的简单法则`,
`简单法则 : 设计、技术、商务、生活的完美融合`,
`交互设计之路 : 让高科技产品回归人性(第二版)`,
`现代通信系统`,
`透析盈利模式 : 魏朱商业模式理论延伸`,
`玩不够的数学 : 算术与几何的妙趣`,
`项目管理之美`,
`乡下人的悲歌`,
`新制度经济学 : 一个交易费用分析范式`,
`要事第一 : 最新的时间管理方法和实用的时间控制技巧`,
`新版一分钟经理人(2015版) : (2015版)`,
`依恋 : 为什么我们爱得如此卑微`,
`七田真超右脑学习法`,
`浴缸里的惊叹 : 256道让你恍然大悟的趣题`,
`学习之道`,
`逻辑与计算机设计基础`,
`人工智能 : 智能系统指南`,
`南京城市史`,
`数学与音乐的创造力 : 捕捉未知与无形`,
`水平营销`,
`思考的技术 : 思考力决定竞争力`,
`宋徽宗时代 : 02 时代拼图`,
`直面大转型时代 : 吴敬琏谈全面深化改革`,
`0～6岁儿童游戏地图 : 给父母的陪玩陪学手册`,
`晚熟的人`,
`变革为何这样难`,
`掌控力：用创业运作系统实现企业卓越运营`,
`24/7:晚期资本主义与睡眠的终结 : 晚期资本主义与睡眠的终结`,
`被掩埋的巨人`,
`腾讯传 : 中国互联网公司进化论`,
`智慧智力创造力`,
`创造力 : 心流与创新心理学`,
`第4消费时代 : 共享经济的新型社会`,
`从包豪斯到我们的豪斯`,
`德国天才1 : 德意志的命运大转折　第三次文艺复兴`,
`CPU自制入门`,
`HR+三支柱：人力资源管理转型升级与实践创新`,
`J2EE核心模式 : 原书第2版`,
`Linux服务器架设、性能调优、集群管理教程 : 实训与项目案例`,
`移动应用UI设计模式（第2版）`,
`编写安全的代码`,
`别错过孩子英语学习敏感期 : 常青藤爸爸教你英语分龄启蒙法`,
`不对称陷阱 : 當別人的風險變成你的風險，如何解決隱藏在生活中的不對等困境`,
`程序员的呐喊`,
`处理器架构`,
`创业的国度 : 以色列经济奇迹的启示`,
`创业无畏 : 指数级成长路线图`,
`代码之美`,
`德国天才1 : 德意志的命运大转折　第三次文艺复兴`,
`腾讯传 : 中国互联网公司进化论`,
`公司进化论 : 伟大的企业如何持续创新`,
`基因之河`,
`杰克.韦尔奇自传（纪念版）`,
`精通Linux驱动程序开发`,
`九败一胜 : 美团创始人王兴创业十年`,
`来份杂碎 : 中餐在美国的文化史`,
`狼蛛`,
`历史的温度 : 寻找历史背面的故事、热血和真性情`,
`普鲁斯特是个神经学家 : 艺术与科学的交融`,
`认识大脑开发大脑 : 认识大脑开发大脑`,
`认知天性 : 让学习轻而易举的心理学规律`,
`软件框架设计的艺术`,
`商业模式的经济解释 : 深度解构商业模式密码`,
`设计原本 : 计算机科学巨匠Frederick P. Brooks的思考`,
`神奇的数学 : 牛津教授给青少年的讲座`,
`挑战程序设计竞赛2 算法和数据结构`,
`图解TCP/IP（第5版）`,
`兔子，跑吧 : 兔子四部曲`,
`喂故事书长大的孩子`,
`嫌疑人`,
`像外行一样思考，像专家一样实践 : 科研成功之道`,
`演说之禅：幻灯片呈现与沟通的艺术（第3版）`,
`游戏改变世界 : 游戏化如何让现实变得更美好`,
`欲望之源 : 从性到钱再到吃：驯服我们的原始本能`,
`技术的本质（经典版） : 技术是什么，它是如何进化的`,
`罗辑思维：认知篇 : 多角度理解认知`,
`名画之谜 : 51幅世界杰作的秘密`,
`难道一切都是我的错吗？ : 重构你的家庭亲密关系`,
`失败的逻辑 : 事情因何出错，世间有无妙策`,
`实践7个习惯 : 改变——生活中的七个习惯 出版时间：2005-5-1`,
`思维规律的研究`,
`只有偏执狂才能生存 : 特种经理人培训手册`,
`注意力 : 专注的科学与训练`,
`自制编译器`,
`知识大融通 : 21世纪的科学与人文`,
`宗教美国，世俗欧洲？ : 主题与变奏`,
`伦敦传`,
`革命之夏 : 美国独立的起源`,
`CSS权威指南（第四版） : Web视觉呈现技术`,
`GitHub入门与实践`,
`GUI设计禁忌2.0`,
`Java I/O (O'Reilly Java)`,
`MIPS处理器设计透视`,
`POSIX多线程程序设计`,
`Python网络数据采集`,
`比一千个太阳还亮 : 原子科学家的故事`,
`苍河白日梦`,
`超级版图 : 全球供应链、超级城市与新商业文明的崛起`,
`程序员面试逻辑题解析`,
`大规模分布式存储系统 : 原理解析与架构实战`,
`动物怪谭`,
`儿童的一百种语言 : 转型时期的瑞吉欧·艾米利亚经验（第3版）`,
`发布！软件的设计与部署`,
`分布式Java应用 : 基础与实践`,
`巨富 : 全球超级新贵的崛起和其他人的没落`,
`理解未来的7个原则 : 如何看到不可见，做到不可能`,
`领导力的5个层次`,
`庞加莱猜想 : 追寻宇宙的形状`,
`培养高效能人士7个习惯的方法`,
`強勢領導的迷思 : 從林肯到歐巴馬，我們到底想要哪一種政治領袖？`,
`让大象飞 : 激进创新，让你一飞冲天的创业术`,
`人性的弱点 : 如何赢取友谊与影响他人`,
`软件系统架构 : 使用视点和视角与利益相关者合作`,
`淘宝十年产品事`,
`体验经济`,
`无从选择 : 为何多即是少`,
`羊皮卷`,
`移动设备交互设计`,
`移动应用的设计与开发`,
`用户体验面面观 : 方法、工具与实践`,
`张学良口述历史(访谈实录)`,
`Android安全攻防权威指南`,
`圣女的救济`,
`数据结构与算法分析 : Java语言描述`,
`软件开发的201个原则`,
`华为能，你也能 : IPD重构产品研发`,
`富足 : 改变人类未来的4大力量`,
`岛上书店`,
`离经叛道 : 不按常理出牌的人如何改变世界`,
`奇点临近 : 当计算机智能超越人类`,
`The Life-Changing Magic of Tidying Up : The Japanese Art of Decluttering and Organizing`,
`住宅设计解剖书`,
`Java Network Programming`,
`LINUX权威指南（第三版）`,
`Living History`,
`Python核心编程（第二版）`,
`Python灰帽子 : 黑客与逆向工程师的Python编程之道`,
`SEO艺术`,
`TCP/IP指南（卷2）`,
`TCP/IP详解 卷1：协议（原书第2版） : 卷1：协议（原书第2版）`,
`卧底经济学`,
`Web Hacking : Attacks and Defense`,
`Web前端黑客技术揭秘`,
`Web安全深度剖析`,
`编写高质量代码 : Web前端开发修炼之道`,
`编写可测试的JavaScript代码`,
`编译原理及实践`,
`博弈论的诡计 : 日常生活中的博弈策略`,
`打开餐巾纸 : 《餐巾纸的背面》之实用手册`,
`产品经理手册`,
`吃掉那只青蛙`,
`持续集成 : 软件质量改进和风险降低之道`,
`道路与梦想 : 我与万科20年`,
`颠覆式创新：移动互联网时代的生存法则`,
`程序设计语言的形式语义`,
`番茄工作法图解 : 简单易行的时间管理方法`,
`高效能人士的第八个习惯 : 从效能迈向卓越`,
`孤独的人都要吃饱 : 孤独时，请给自己最好的安慰。`,
`管理十诫 : 影响你一生的管理哲学`,
`好好学习 : 个人知识管理精进指南`,
`精要主义 : 如何应对拥挤不堪的工作和生活`,
`离经叛道 : 不按常理出牌的人如何改变世界`,
`黎明之街`,
`理性的儀式 : 文化、協調與共通認知`,
`普利策传`,
`抢占心智 : 产品与品牌快速崛起的引爆打法`,
`轻公司 : 互联网变革中国制造`,
`情商 : 为什么情商比智商更重要`,
`日本电子产业兴衰录`,
`如何成为一名优秀的言情女主`,
`如何创造思维 : 人类思想所揭示出的奥秘`,
`设计师要懂心理学`,
`深入分析Linux内核源代码`,
`提高C++性能的编程技术`,
`弦理论`,
`享受图画书`,
`信任的速度 : 一个可以改变一切的力量`,
`引爆点 : 如何制造流行`,
`应用逻辑`,
`用户体验与可用性测试`,
`有趣的二进制 : 软件安全与逆向分析`,
`有效的单元测试`,
`运动改变大脑`,
`在线 : 数据改变商业本质，计算重塑经济未来`,
`普利策传`,
`花千骨（上下）`,
`领导力教练（实践篇）`,
`奇点临近 : 当计算机智能超越人类`,
`能力陷阱`,
`深夜小狗神秘习题`,
`数据结构与算法之美`,
`数值分析导论`,
`数字设计原理与实践`,
`谁动了我的奶酪?`,
`指数型组织 : 打造独角兽公司的11个最强属性`,
`终身成长 : 重新定义成功的思维模式`,
`自制搜索引擎`,
`计算机是怎样跑起来的`,
`世界是平的（3.0版） : 21世纪简史`,
`事业合伙人：知识时代的企业经营之道（精装）`,
`数据结构、算法与应用 : C++语言描述`,
`弹性计算：无处不在的算力（全彩）`,
`社交媒体简史 : 从莎草纸到互联网`,
`管理：使命、责任、实践（使命篇）`,
`沸腾的岁月`,
`美国制造 : 国家繁荣为什么离不开制造业`,
`食帖12：厨房，治愈人生的避难所`,
`IT不再重要 : 互联网大转换的制高点——云计算`,
`Java Nio`,
`JavaScript王者归来`,
`Java并发编程的艺术`,
`品格之路`,
`UML用户指南`,
`百岁人生：长寿时代的生活和工作 : 长寿时代的生活和工作`,
`藏地密码1`,
`从0到1 : 开启商业与未来的秘密`,
`大道至简 : 德国零售巨头ALDI管理法`,
`大话存储Ⅱ : 存储系统架构与底层原理极限剖析`,
`大数据时代 : 生活、工作与思维的大变革`,
`第3选择 : 解决所有难题的关键思维`,
`赋能 : 打造应对不确定性的敏捷团队`,
`高扩展性网站的50条原则`,
`格局`,
`古怪的身体 : 时尚是什么`,
`随机生存的智慧 : 黑天鹅语录`,
`吉田医生哈佛求学记`,
`开放式组织 : 面向未来的组织管理新范式`,
`可能与不可能的边界 : P/NP问题趣史`,
`兰海说成长 : 懂得孩子，才能更好帮助孩子`,
`品格之路`,
`平台战略 : 正在席卷全球的商业模式革命`,
`七周七并发模型`,
`清醒思考的艺术 : 你最好让别人去犯的52种思维错误`,
`丘吉尔自传 ——我的早年生活`,
`天书夜读 : 从汇编语言到Windows内核编程`,
`投资者的敌人`,
`托尔斯泰传`,
`危机领导力 : 领导团队解决危机的十种方法`,
`稀缺 : 我们是如何陷入贫穷与忙碌的`,
`一个瑜伽行者的自传 : 60周年纪念版全文译本`,
`应需而变 : 为瞬息万变的世界提供伟大的产品和服务`,
`运营之光3.0 : 我的互联网运营方法论与自白`,
`走入IBM小型机世界`,
`AKB48 Group官方访谈集 : 星光的起点与纪念`,
`反模式 : 危机中软件架构和项目的重构`,
`马可·波罗游记`,
`美国制造 : 国家繁荣为什么离不开制造业`,
`数学思维导论 : 学会像数学家一样思考`,
`刷新 : 重新发现商业与未来`,
`专注力 : 心流的惊人力量`,
`專注力就是你的超能力 : 掌控自我、提升成績的18個學習武器`,
`专注力：如何高效做事`,
`专业主义`,
`变量2 : 推演中国经济基本盘`,
`富豪的心理 : 财富精英的隐秘知识`,
`客户成功：持续复购和利润陡增的基石`,
`从偶然到必然：华为研发投资与管理实践 : 华为研发投资与管理实践`,
`华为研发`,
`威权政治`,
`郁金香热`,
`百年商业头条 : 从《纽约时报》窥见美国商业繁荣之路`,
`机器人时代`,
`小而美的家`,
`Intel微处理器`,
`OKR工作法 : 谷歌、领英等公司的高绩效秘籍`,
`Python参考手册（第4版）`,
`爱的流放地`,
`北京折叠`,
`不可预知时代的可预知结果`,
`参与感 : 小米口碑营销内部手册`,
`持续的幸福`,
`创业时, 我们在知乎聊什么?`,
`达·芬奇密码`,
`低风险创业`,
`断舍离`,
`孵化Twitter : 从蛮荒到IPO的狂野旅程`,
`父母手记:教育好孩子的101种方法`,
`架构真经 : 互联网技术架构的设计原则（原书第2版）`,
`精进 : 如何成为一个很厉害的人`,
`開發精確的思考`,
`联想风云`,
`你的灯亮着吗? : 发现问题的真正所在`,
`贫穷的本质（修订版） : 我们为什么摆脱不了贫穷`,
`情感机器 : 人类思维与人工智能的未来`,
`认知盈余：自由时间的力量`,
`软件架构的艺术`,
`软件架构师应该知道的97件事`,
`统计数字会撒谎`,
`万物解释者（畅销升级版） : 复杂事物的极简说明书`,
`一万小时天才理论`,
`智能Web算法`,
`致未来的你——给女孩的十五封信`,
`祝你生日快乐`,
`助推 : 如何做出有关健康、财富与幸福的最佳决策`,
`断舍离`,
`10人以下小团队管理手册`,
`海盗传奇`,
`客户说：如何真正为客户创造价值（精装）`,
`如果巴西下雨，就买星巴克股票`,
`企鹅与怪兽 : 互联时代的合作、共享与创新模式`,
`Java编程风格`,
`OKR：源于英特尔和谷歌的目标管理利器 : 源于英特尔和谷歌的目标管理利器`,
`彼得原理 : 彼得原理`,
`程序员的数学`,
`催眠术入门`,
`大繁荣 : 大众创新如何带来国家繁荣`,
`代码优化 : 有效使用内存`,
`发现你的天赋 : 天分与热情成就幸福人生`,
`发现商业模式`,
`黑客与设计：剖析设计之美的秘密`,
`解密无印良品`,
`可复制的领导力 : 樊登的9堂商业课`,
`環境理性與制度抉擇`,
`人才盘点完全应用手册`,
`人工智能狂潮 : 机器人会超越人类吗？`,
`商业冒险 : 华尔街的12个经典故事`,
`深度学习 : 智能时代的核心驱动力量`,
`淘宝技术这十年 : 淘宝技术大学校长解密淘宝十年`,
`天生犯罪人`,
`伟大的工作，伟大的事业`,
`夏天、烟火和我的尸体`,
`响应式Web设计 : HTML5和CSS3实战`,
`小兵传奇（1）・银河禁锢`,
`引爆用户增长`,
`晕眩年代 : 1900-1914年西方的变化与文化`,
`超级运营术`,
`汇编语言编程艺术`,
`流量池`,
`民族进化的心理定律`,
`生命清单`,
`数据结构C++语言描述`,
`数据资本时代`,
`水煮三国`,
`证明达尔文 : 进化和生物创造性的一个数学理论`,
`下一个倒下的会不会是华为（珍藏版）`,
`His Bloody Project`,
`谁在收藏中国 : 美国猎获亚洲艺术珍宝百年记`,
`CSS禅意花园（修订版）`,
`人工智能的未来`,
`Linux嵌入式系统高级程序设计`,
`Linux高级程序设计`,
`白帽子讲浏览器安全`,
`东京大学通识讲座2 : 热血篇`,
`疯狂Android讲义`,
`工作漂流`,
`海底捞你学不会`,
`简素 : 日本文化的底色`,
`雷电之书 : 解密自然与生命的原始能量`,
`袍哥 : 1940年代川西乡村的暴力与秩序`,
`权力的终结 : 权力正在失去,世界如何运转`,
`天才在左，疯子在右`,
`这就是OKR : 让谷歌、亚马逊实现爆炸性增长的工作法`,
`ACM图灵奖`,
`Android高级编程`,
`媒介即讯息及其他50条荒谬的广告法则`,
`每天懂一点色彩心理学`,
`深入理解Linux虚拟内存管理`,
`生死昆虫记 : 影响历史的人虫大战`,
`重构 : 数字化转型的逻辑`,
`中国电竞幕后史`,
`价值 : 我对投资的思考`,
`超预测 : 预见未来的艺术和科学`,
`为什么受伤的总是聪明人`,
`Intel Threading Building Blocks编程指南`,
`Java线程`,
`编写高质量代码 : 改善JavaScript程序的188个建议`,
`产品思维：从新手到资深产品人`,
`创京东 : 刘强东亲述创业之路`,
`大话存储 : 网络存储系统原理精解与最佳实践`,
`大脑使用说明书 : 大脑使用说明书`,
`对标准答案说不：试卷中的周国平`,
`对象设计 : 角色.责任和协作`,
`给孩子的世界地理大数据（全六册）`,
`公共管理论丛`,
`故事的领导力 : 领导力语言的终极秘密！`,
`国家为什么会失败`,
`焦虑星球笔记 : 活下去的理由续篇`,
`金钱有术 : 金钱有术`,
`精简 : 无印良品与品牌理念打造`,
`精益求精 : 卓越的互联网产品设计与管理`,
`你的记忆怎么了？`,
`培思的力量 : 产品及周期优化法在产品开发中的应用`,
`恰如其分的软件架构 : 风险驱动的设计方法`,
`网站设计解构 : 有效的交互设计框架和模式`,
`向毛泽东学管理`,
`性格的力量 : 勇气、好奇心、乐观精神与孩子的未来`,
`硬件安全攻防大揭秘`,
`超级聊天术`,
`滑动解锁 : 解密技术基因，揭秘数字世界`,
`神一样的产品经理 : 基于移动与互联网产品实践`,
`睡眠的科学 : 生命入睡、苏醒的机制与奥秘`,
`孙悟空是个好员工`,
`至关重要的设计`,
`察知力 : 中村俊辅：任意球大师的“察知力”`,
`专注力 : 化繁为简的惊人力量`,
`帮助你的孩子爱上阅读`,
`The World According to Star Wars`,
`茶叶大盗 : 改变世界史的中国茶`,
`逆转 : 弱者如何找到优势，反败为胜`,
`HTML与XHTML权威指南`,
`Java加密与解密的艺术`,
`Linux内核API完全参考手册`,
`Python网络编程基础 : 使用Python构建网络程序的综合指南`,
`Spark技术内幕 : 深入解析Spark内核架构设计与实现原理`,
`大学的终结 : 泛在大学与高等教育革命`,
`高效团队开发 : 工具与方法`,
`孤独深处`,
`国际政治的社会演化 : 从公元前8000年到未来`,
`架构之美：行业思想领袖揭秘软件设计之美（评注版）`,
`家用游戏机简史`,
`开放空间引导技术：集思广益，解决冲突，达成共识，实现自组织的高效方法（第3版）`,
`图解OpenFlow`,
`图解性能优化`,
`超越你的大脑：控制情绪、优化决策 : 控制情绪、优化决策 、增进关系、激发团队`,
`成事 : 冯唐品读曾国藩嘉言钞`,
`麦肯锡方法：用简单的方法做复杂的事`,
`美第奇效应`,
`墨菲定律`,
`输赢心理学 : 为什么想赢的人会输,为什么怕输的人会赢`,
`知识的边界`,
`终极算法 : 机器学习和人工智能如何重塑世界`
];



(async () => {
    /*
    http://c.diaox2.com/view/app/?m=show&id=1218&ch=firstpage&from=timeline&isappinstalled=1
    https://m.igetget.com/share/course/article/article_id/86035?from=timeline
    https://www.digitaling.com/articles/168020.html?from=timeline&isappinstalled=0
    https://www.jianshu.com/p/a56b7dd85489
    */
    //

    let n = 0;

    // 数据去重
    const UNREPEATED_BOOK_LIST =  Array.from(new Set(REPEATED_BOOK_LIST));

    let NEW_BOOK_LIST = [];
    let RETRY_BOOK_LIST = [];

    // 去重后数据写入文档
    fs.writeFileSync(path.resolve(__dirname, './book/book_list.txt'), UNREPEATED_BOOK_LIST.join('\n'), 'utf-8');

    // 每隔十秒进行一次数据拉取
    let tid = setInterval(async function(){
        // 如果是最后一个就退出定时器，同时将新图书列表写入文件，以及需要重试的记录写入文件
        if(n == UNREPEATED_BOOK_LIST.length) {
            fs.writeFileSync(path.resolve(__dirname, './book/new_book_list.txt'), NEW_BOOK_LIST.join('\n'), 'utf-8');
            fs.writeFileSync(path.resolve(__dirname, './book/retry_book_list.txt'), RETRY_BOOK_LIST.join('\n'), 'utf-8');
            
            //for(let i = 0;i < 10;i++) console.log(NEW_BOOK_LIST[i])

            clearInterval(tid);
            return;
        }

        // 获取一条书名
        const title = UNREPEATED_BOOK_LIST[n];

        console.log(`DEBUG:`, ((n + 1) / (UNREPEATED_BOOK_LIST.length) * 100).toFixed(2) + '%', n, UNREPEATED_BOOK_LIST.length, title);

        // 根据书名获取相应的记录
        const data = await extractBookList({
            url: `https://search.douban.com/book/subject_search?search_text=${encodeURIComponent(title)}&cat=1001`,
            originTitle: title,
            jsRender: true
        });

        // 如果获取到数据就写入列表，失败就写入重试列表
        if(data)
            NEW_BOOK_LIST.push(`${data.title}#${data.href}#${data.rating_nums}#${data.pl}#${data.meta}`);
        else
            RETRY_BOOK_LIST.push(title);

        n++;

    }, 10000);

})();

